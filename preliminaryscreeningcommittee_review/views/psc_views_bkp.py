    ...
    try:
        ...
        if _branch in region_list:
            ...
            pscall_accounts = CustomerTable.objects.filter(
                Q(review_type='psc') | Q(review_type__isnull=True),npa_status=True,branch_code__in=list(ro_list)
            ).annotate(
                status_order=Case(
            ...
            sacall_accounts = CustomerTable.objects.filter(npa_status=True,branch_code__in=list(ro_list),sac_details__isnull=False).annotate(
                    status_order=Case(
            ...
            sacall_accountsApproval = CustomerTable.objects.filter(npa_status=True,branch_code__in=list(ro_list),sac_details__isnull=False).annotate(
                    status_order=Case(
        ...
        # HO Data
            ...
            sacall_accounts = sacall_accounts = CustomerTable.objects.filter(npa_status=True,branch_code__in=list(ro_list),sac_details__isnull=False).annotate(
                    status_order=Case(
        ...
        # BO Data
            ...
            print('pscall_accounts',pscall_accounts.count(), time.time()- bo_start_time) 
            pscfiltered_accounts = []
            for account in pscall_accounts:
                # print(account.branch_code)
                if account.branch_code:
            ...
            print(f"data of maker loaded for bo time {time.time()-bo_start_time}")
            pscall_accountsApproval = CustomerTable.objects.filter(
                Q(review_type='psc') | Q(review_type__isnull=True),npa_status=True
            ).annotate(
                status_order=Case(
            ...
            psccustomer_dataApproval = pscfiltered_accountsApproval
            print(f"data of checker loaded for bo time {time.time()-bo_start_time}")
...
def convertToYYYYMMDD(date_str):
    ...
    if date_str in ["NO_DATA", "NULL", "null", "NA"]:
        return "01-01-1990"
    else:
...
def date_slash_2_hyphen(dateStr):
    ...
    if dateStr in ["NO_DATA", "NULL", "null", "NA"]:
        return "01-01-1990"
...
def api_datatype_check(data):
    if data in ["NO_DATA", "NULL", "null", "NA"]:
        return "01-01-1990"
    else:
        return data
            ...
            if review == 'PSC1':
                ...
                for cust_id in cust_ids:
                    ...
                    if total_exposure and float(total_exposure['total_exposure']) >= psc_mom_limit:
                        ...
                        messages.success(request, "MOM has been closed successfully")
                        MOMuser_log = {
                            'user': request.session['emp_id'],  
                            'ses_key': sessionKey,
                            'designation': request.session['designation'],
