...
def dashboard(request):
    ...
    try:
        ...
        if _branch in region_list:
            ...
            ro_start_time = time.time()
            ro_list = BranchMaster.objects.filter(zone=_branch).values_list('branch_code',flat=True).distinct()
            ro_sl  = BranchMaster.objects.filter(zone=_branch).values_list('sl_no',flat=True).distinct()
            pscall_accounts = CustomerTable.objects.filter(
                Q(review_type='psc') | Q(review_type__isnull=True),npa_status=True,branch_code__in=list(ro_sl)
            ).annotate(
                status_order=Case(
            ...
            # print(pp,pscall_accounts, _branch, list(ro_list), type(_branch), type(list(ro_list)[45]), (_branch in list(ro_list)))           
            ...
            for account in pscall_accounts:
                if account.branch_code:
                    ...
                    # pscfiltered_accounts.append(account)
                    exists_in_psc = PSCTable.objects.filter(
                        cust_id = account.cust_id,
                        branch_code_id__branch_code=request.session['branch_code']
                    ).exists()
                    if exists_in_psc:
                        pscfiltered_accounts.append(account)
            ...
            sacall_accounts = CustomerTable.objects.filter(npa_status=True,branch_code__in=list(ro_sl),sac_details__isnull=False).annotate(
                    status_order=Case(
            ...
            sacall_accountsApproval = CustomerTable.objects.filter(npa_status=True,branch_code__in=list(ro_sl),sac_details__isnull=False).annotate(
                    status_order=Case(
            ...
            ro_load_time = time.time()-ro_start_time
            print("User Id: {}, Designation: {}, Role: {}, Action: {}, Message: RO Dashboard Opened, Loading Time: {}".format(request.session['emp_id'], request.session['designation'], session_role, "RO Dashboard", ro_load_time))
        # HO Data
        elif _branch == '001':
            print('HO region')
            #HO Maker/Checker Dashboard
            ho_start_time = time.time()
            ro_list = BranchMaster.objects.values_list('branch_code',flat=True).distinct()
            ro_sl  = BranchMaster.objects.filter(zone='001').values_list('sl_no',flat=True).distinct()
            # print(ro_sl)
            pscall_accounts = CustomerTable.objects.filter(
                Q(review_type='psc') | Q(review_type__isnull=True),npa_status=True,current_role__in=('HO Maker', 'HO Checker','Convener')
            ).annotate(
                status_order=Case(
            ...
            psccustomer_dataApproval = pscfiltered_accountsApproval
            #print('psccustomer_data',psccustomer_data)
            sacall_accounts = sacall_accounts = CustomerTable.objects.filter(npa_status=True,sac_details__current_role__in=('HO Maker', 'HO Checker', 'Convener'),sac_details__isnull=False).annotate(
                    status_order=Case(
            ...
            for account in sacall_accounts:
                if account.branch_code:
                    ...
                    sacfiltered_accounts.append(account)
            saccustomer_data = sacfiltered_accounts
            sacall_accountsApproval = CustomerTable.objects.filter(npa_status=True,sac_details__current_role__in=('HO Maker', 'HO Checker', 'Convener'),sac_details__isnull=False).annotate(
                    status_order=Case(
            ...
            ho_load_time = time.time() - ho_start_time
            print("User Id: {}, Designation: {}, Role: {}, Action: {}, Message: HO Dashboard Opened, Loading Time: {}".format(request.session['emp_id'], request.session['designation'], session_role, "HO Dashboard", ho_load_time))
        ...
        # BO Data
            ...
            # print(f"psc data of maker loaded for bo time {time.time()-bo_start_time}")
            ...
            print(psccustomer_dataApproval)
            # print(f"psc data of checker loaded for bo time {time.time()-bo_start_time}")
        ...
        if 'psc001maker2' in roles or 'psc001checker2' in roles:
            ...
            sac_data_export = sac_data_export.filter(Q(current_role__in=['HO Maker', 'HO Checker', 'Convener']) |
                    Q(status='Convened'))
            print("overall dashboard loading time", time.time()-_start_time)
...
def fetch_employee_data(request):
    ...
    if request.is_ajax() and request.method == 'POST':
        ...
        #print('staff_no',staff_no,staff_no[0])
        # if staff_no[0]
        try:
            if staff_no:
                staff_no = 'K' + staff_no[1:] if staff_no else staff_no
                if is_api_data == True:
                    ...
                    # print('encHUPMINQ',encHUPMINQ, encHUPMINQ['empName'])
                    if len(encHUPMINQ) > 2:
                        #print('in 1st if')
                        if staff_no[1:] == encHUPMINQ['empId'] and not encHUPMINQ['empName'] == {}:
...
def convertToYYYYMMDD(date_str):
    ...
    if date_str in ["NO_DATA", "NULL", "null", "NA", "None", None]:
        return "01-01-1990"
    else:
...
def date_slash_2_hyphen(dateStr):
    ...
    if dateStr in ["NO_DATA", "NULL", "null", "NA", "None", None] or not dateStr:
        return "01-01-1990"
    ...
    for dtft in date_format:
        try:
            dateObj = datetime.strptime(dateStr, dtft)
...
def api_datatype_check(data):
    if data in ["NO_DATA", "NULL", "null", "NA", "None", None]:
        return "01-01-1990"
    else:
        return data
...
def mom_date_fetch(request, pk):
    ...
    non_active_mom_prior = datetime(2025, 1, 1)
    if active_mom_prior:
...
def append_lapse_details_CustTable(request,cust_id, present_head_data, lapses_data, facility_no, review_type):
    # print("cust_id", cust_id)
    # print("present_head_data", present_head_data)
    # print("lapses_data", lapses_data)
    # print("facility_no", facility_no)
    # print("review_type", review_type)
    try:
        customer = CustomerTable.objects.get(cust_id=cust_id)
        # print("json save ",cust_id, customer)
    except Customer.DoesNotExist:
        print(f"Customer with ID {cust_id} not found")
        return False

    lapse_data = customer.lapses_details or {
        "cust_id": cust_id,
        "records": [
                    {
                        "staff_no": [],
                        ...
                        "staff_designation": [],
                        "staff_branch_code": [],
                        "staff_lapses_details": []
                    }
                ] 
            }
    record = lapse_data["records"][0]
    if review_type == "PSC1" or review_type == "PSC2":
        # add staff data if not already exists
        # if present_head_data["psc_branch_head_id"] not in record["staff_no"]:
        record["staff_no"].append(present_head_data["psc_branch_head_id"])
        record["staff_name"].append(present_head_data["psc_branch_head_name"])
        record["staff_designation"].append(present_head_data["psc_branch_head_desig"])
        record["staff_branch_code"].append(present_head_data["psc_branch_head_sol_id"])
        
        for lapse in lapses_data:
            record["staff_lapses_details"].append({
                "acc_no": facility_no,
                "emp_code": lapse["lapses_psc_branch_head_id"],
                "lapses_observed": lapse["lapses_psc_remarks"]
            })

        customer.lapses_details = lapse_data
        customer.save()
        return True
    elif review_type == "sac" or review_type == "SAC":
        # add staff data if not already exists
        # if present_head_data["psc_branch_head_id"] not in record["staff_no"]:
        record["staff_no"].append(present_head_data["psc_branch_head_id"])
        record["staff_name"].append(present_head_data["psc_branch_head_name"])
        ...
        if request.is_ajax() and request.method == 'POST':
            ...
            if dd_select == 'no':
                ...
                if review_type == 'PSC1' or review_type == 'PSC2':
                    ...
                    psc_facility_no = PSCTable.objects.filter(id=pk).values('facility_num')[0]['facility_num']
                ...
                if review_type == 'PSC1' or review_type == 'PSC2':
                    ...
                    psc_mom_count = PSCTable.objects.filter(psc_rec_id = psc_rec_id_,mom_lapse__isnull=False).count()  
                    psc_cust_id = PSCTable.objects.filter(id=pk).values('cust_id')[0]['cust_id'] 
                    # print("json_lapses_heads: ",json_lapses_heads)
                    # print("json_present_head: ",json_present_head)
                    # print("rec_id: ",psc_rec_id_)
                    # print("facility_no: ",psc_facility_no)
                    append_lapse_details_CustTable(request,psc_cust_id, json_present_head[0], json_lapses_heads, psc_facility_no, review_type)
                    # return
                    ...
                    sac_rec_id_ = SACTable.objects.filter(id=pk).values('sac_rec_id')[0]['sac_rec_id']
                    sac_facility_num = SACTable.objects.filter(id=pk).values('psc_rec_id__facility_num')[0]['psc_rec_id__facility_num']
                    sac_acc_total = SACTable.objects.filter(sac_rec_id = sac_rec_id_).count()    
                    sac_mom_count = SACTable.objects.filter(sac_rec_id = sac_rec_id_,mom_lapse__isnull=False).count()  
                    sac_cust_id = SACTable.objects.filter(id=pk).values('psc_rec_id__cust_id')[0]['psc_rec_id__cust_id'] 
                    #print(sac_rec_id_,sac_acc_total,sac_mom_count,sac_cust_id)
...
def mom_close(request):
    ...
    try:
        ...
        if request.is_ajax() and request.method == 'POST':
            ...
            mom_from_date, mom_to_date = mom_date_fetch(request, pk)
            meet_id = MOMTable.objects.filter(id=pk).values('meeting_id')[0]['meeting_id']
            ...
            if not cust_ids:
                ...
                print("User Id: {}, Designation: {}, Role: {}, Action: {}, Message: {}".format(request.session['emp_id'], request.session['designation'], session_role, "PSC MOM Close","PSC MOM has been closed"))
                mom_instancelog = MOMTable.objects.get(id=pk)
                update_PSC_SAC_MOM_logs(mom_instancelog, 'close', MOMuser_log,'MOM')
                # messages.success(request, f"MOM ({meet_id}) has been closed successfully")
            ...
            if review == 'PSC1':
                ...
                print("User Id: {}, Designation: {}, Role: {}, Action: {}, Message: {}".format(request.session['emp_id'], request.session['designation'], session_role, "SAC MOM Close","SAC MOM has been closed"))
                mom_instancelog = MOMTable.objects.get(id=pk)
                update_PSC_SAC_MOM_logs(mom_instancelog, 'close', MOMuser_log,'MOM')
